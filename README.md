...
### 功能
- **整段解析**：粘贴一整段文字，自动标出其中全部拼音首字母缩写，点击缩写就地查看候选解释，一键复制带解释的完整译文。
- **我的词库**：自动记录查询历史，支持收藏缩写、添加自定义解释（展示时优先于接口结果）、删除单条、清空历史与一键清空，词库可通过 JSON 文件导入 / 导出备份。
- **划词浮窗升级**：选中长句时批量列出全部缩写，点击任意结果即可复制；浮窗支持 `↑` `↓` 切换、回车复制、`Esc` 关闭；与网页版共用同一份本地词库与历史记录。
- **深色模式**：默认跟随系统外观，右上角按钮可在 跟随系统 → 浅色 → 深色 间切换，刷新后记住选择。

> 词库保存在浏览器本地（localStorage）。网页版与同浏览器内安装的用户脚本共享同一份数据；在浏览器或设备之间迁移时可使用词库的「导出 / 导入」功能。
...
[Flow Launcher 插件](https://github.com/dragonish/Flow.Launcher.Plugin.Nbnhhsh) @dragonish
